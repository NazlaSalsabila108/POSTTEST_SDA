//karena nim genap, maka pengerjaan posttest ini mengikuti intruksi untuk soal genap
...
void sisipJadwal(){
    int posisi = 9; //karena digit terakhir nim adalah (8) + 1 = 9 jadi otomatis penyisipan hanya dilakukan di posisi 9
...
int main () {
    ...
    while (true) {
        cout << "Pilih Menu: ";
        if (cin >> pilihan && pilihan >= 0 && pilihan <= 5) {
            cin.ignore(); 
            break;
        } else {
            cout << RED << "Input tidak valid. Silakan masukkan angka antara 0-5.\n" << RESET;
            cin.clear();
            cin.ignore(100,'\n');
        }
    }
